...
### GET THE LAT/LON INTERSECTION OF THE MISR AND EPIC DATA
    ...
    print("MISR")
    good = (misr_lat > -90.)
    print(np.amin(misr_lat[good]))
    print(np.amax(misr_lat[good]))
    
    print()
    print("EPIC")
    good = (epic_lat > -90.)
    print(np.amin(epic_lat[good]))
    print(np.amax(epic_lat[good]))
    
# Convert the MISR AODs at 550 nm to 443 nm to compare with EPIC MAIAC retrievals

    print(np.shape(misr_aod_raw_scale_sub))
    print(error)
